...
{
    // Büyük N senaryolarında aynı anda saldırmayı önlemek için
    if (p->id % 2 == 0)
        usleep(1000 * (p->id % 10)); 
        // her 10 filozof farklı delay alır: 0,1,2,...9 ms
...
static void philo_eat(t_philo *p)
{
    ...
    p->last_meal_time = get_time_ms();   // çatal aldıktan hemen sonra güncelle
    p->meals_eaten++;
